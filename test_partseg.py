...
def main(args):
    ...
    with torch.no_grad():
        ...
        for batch_id, (points, label, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader),
            ...
            print(points.permute(0, 2, 1)[0].detach().cpu().numpy().shape)
            pcd1 = open3d.geometry.PointCloud()
            pcd1.points = open3d.utility.Vector3dVector(points.permute(0, 2, 1)[0].detach().cpu().numpy())
            pcd1.colors = colos
            open3d.visualization.draw_geometries([pcd1],  # 待显示的点云列表
                                                 window_name="点云显示",
                                                 point_show_normal=False,
                                                 width=800,  # 窗口宽度
                                                 height=600)
            ...
            print(44444,cur_pred_val.shape)
            print(cur_pred_val)
